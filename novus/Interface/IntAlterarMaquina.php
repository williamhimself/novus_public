...
<body>
    <div class="d-flex" id="wrapper">
        ...
        <div id="page-content-wrapper">
            ...
            <div id="content" class="container">
                <h3 id="main-page-form-title">Alteração de Maquina</h3>
                <div class="son-form card">
                   			<?php
                                ...
                                $listaMaquina = unserialize($_SESSION['listaMaquina'][0]);
                                $listaArquivo = unserialize($_SESSION['listaMaquina'][1]);
                       		?>
					<form name="cadMaquina" id="cadMaquina" enctype="multipart/form-data" action="../controle/CadMaquina.php?op=alterar&id=<?php echo $listaMaquina[0]->id_maquina;?>" method="post">
                        <div class="card-body">
                            <div class="form-group">
                                <label for="">Descrição</label>
                                <input type="text" name="descricao" id="descricao"  class="form-field son-form-field form-control" value="<?php echo $listaMaquina[0]->descricao; ?>" required>    
                                <hr>
                            </div>
                            
                            <div class="form-group">
                                <label for="">caracteristica</label>
                                <input type="text" name="caracteristica" id="caracteristica"  class="form-field son-form-field form-control" value="<?php echo $listaMaquina[0]->caracteristica;?>" required>    
                                <hr>
                            </div>
                            <div class="form-group">
                                <label for="">patrimonio</label>
                                <input type="text" name="patrimonio" id="patrimonio"  class="form-field son-form-field form-control" value="<?php echo $listaMaquina[0]->patrimonio;?>" required>    
                                <hr>
                            </div>
                            <div class="form-group">
                                <label for="">Data Próxima Manutenção</label>
                                <input type="date" name="data_proxima_manutencao" id="data_proxima_manutencao"  class="form-field son-form-field form-control" value="<?php echo date('Y-m-d',strtotime($listaMaquina[0]->data_proxima_manutencao));?>" required>    
                                <hr>
                            </div>
                            <div class="form-group">
                                <label for="">Data Aviso Antes</label>
                                <input type="date" name="data_aviso_antes" id="data_aviso_antes"  class="form-field son-form-field form-control" value="<?php echo date('Y-m-d',strtotime($listaMaquina[0]->data_aviso_antes));?>" required>    
                                <hr>
                            </div>
                            <div class="form-group">
                                <label for="">Email para Manutencao</label>
                                <input type="email" name="email_aviso_manutencao" id="email_aviso_manutencao"  class="form-field son-form-field form-control" value="<?php echo $listaMaquina[0]->email_aviso_manutencao;?>" required>    
                                <hr>
                            </div>
                            ...
                            <?php
                                if(!is_null($listaArquivo[0]->descricaoArquivo)){
                                    for($i = 0; $i < count($listaArquivo); $i++){
                                        echo '<a href="../Arquivos/'.$listaArquivo[$i]->arquivo.'"download> Download Arquivo
                                              </a>
                                              <input type="checkbox" name="excluirArquivo['.$listaArquivo[$i]->id_arquivo.']" value="'.$listaArquivo[$i]->arquivo.'">Exluir Arquivo';
                                        echo '<div class="form-group">
                                                <label for="">Descricao Arquivo</label>
                                                <input type="text" name="descricaoArquivo['.$listaArquivo[$i]->id_arquivo.']" class="form-field son-form-field form-control" value="'.$listaArquivo[$i]->descricaoArquivo.'">
                                                <hr>
                                            </div>
                        ...
                        <div class="card-footer">
                            <div class="confirm-btns">
                                <button name="btneditar" id="btneditar" value="Editar" class="btn btn-primary">Editar Maquina</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <script>
		...
		function adicionaArquivo(){
			...
			+'<div class="form-group">'
                                +'<label for="">Descricao Arquivo</label>'
                                +'<input type="text" name="descricaoArquivo[]" class="form-field son-form-field form-control">'
                                +'<hr>'
                            +'</div>'
				+'<span class="input-goup-addon" id="basic-addonl">selecione o Arquivo: </span>'
				+'<input id="inputArquivo" name="inputArquivo[]" type="file" class="arquivo" multiple data-show-upload="false" data-show-caption="true">'
